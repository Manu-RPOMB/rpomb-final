...
import { TrendingUp, FileSpreadsheet, Truck, Plus, Trash2, ChefHat, Edit3, LogOut, ShoppingBag, CheckSquare, Send, Clock, Upload, AlertTriangle, X, Search, List, FileText, ChevronLeft, Package, Grid, AlertCircle, Calendar, Settings, Euro, Calculator, Banknote, RefreshCw, ArrowRightLeft, Sparkles, ShieldAlert, UserCheck } from 'lucide-react';
...
// --------------------------------------------------------
// 👇 TES CLÉS (J'ai remis celles de ton message) 👇
// --------------------------------------------------------
...
const GEMINI_API_KEY = "AIzaSyCXY1yNb0j2U30a68YvXIq9pWhJkvcERyI"; // ⚠️ COLLE TA CLÉ GOOGLE ICI
// --------------------------------------------------------
...
// MISE A JOUR DES FAMILLES DEMANDEE
const FAMILLES_LIST = ['Tout', 'Frais', 'Surgelé', 'Epicerie', 'Légumes', 'Viande', 'Emballage', 'Entretien', 'Divers'];
...
export default function App() {
  ...
  async function fetchStock() { 
      ...
      const { data } = await supabase.from('ingredients').select('*').eq('is_validated', true).order('nom'); 
      if (data) setIngredients(data); 

      // Récupère les ingrédients en attente pour l'admin
      const { data: pending } = await supabase.from('ingredients').select('*').eq('is_validated', false);
  ...
  const askGeminiIdeas = async () => {
      ...
      const prompt = `Agis comme un chef de cuisine collective. Voici mon stock : ${restes}. 
  ...
  const validerIngredientAdmin = async (id, valide) => {
      if (valide) {
          ...
          if(confirm("Supprimer la demande ?")) await supabase.from('ingredients').delete().eq('id', id);
      }
      fetchStock();
  };
  ...
  const validerPerte = async () => {
      ...
      await supabase.from('mouvements_stock').insert({ ingredient_id: ing.id, type_mouvement: 'Perte', quantite: parseFloat(perteData.qte), motif: `${perteData.motif} ${perteData.comment ? `(${perteData.comment})` : ''}`, valeur_perte: valeurPerdue });
      alert(`🗑️ Perte enregistrée !`);
      setShowPerteModal(false); setPerteData({ id: '', qte: '', motif: 'Périmé', comment: '' }); fetchStock();
  };
  ...
  const validerTransformation = async () => {
      ...
      await supabase.from('mouvements_stock').insert({ ingredient_id: srcIng.id, type_mouvement: 'Transformation Sortie', quantite: parseFloat(transfoData.srcQte), motif: `Transformé en ${destIng.nom}` });
      alert(`♻️ Transformation validée !`);
      setShowTransfoModal(false); setTransfoData({ srcId: '', srcQte: '', destId: '', destQte: '' }); fetchStock();
  };
  ...
  const creerRecette = async () => {
      ...
      if (data) {
          setRecettes([...recettes, data]); setIsCreatingRecette(false); setNewRecetteName('');
          const { data: details } = await supabase.from('recette_ingredients').select('*, ingredients(*)').eq('recette_id', data.id);
          setRecetteDetails(details || []); setSelectedRecette(data);
      }
  };
  const ajouterIngredientAuPlat = async () => {
      ...
      if (!error) {
          ...
          setRecetteDetails(data || []); setAjoutIngredientRecette(''); setQteAjoutRecette('');
      }
  };
        ...
        {user.role === 'admin' && pendingIngredients.length > 0 && (
            <div className="mb-6 animate-in slide-in-from-top">
                <div className="bg-amber-100 border-l-4 border-amber-500 p-4 rounded-r-xl shadow-sm">
                    <h3 className="font-bold text-amber-900 flex items-center gap-2"><ShieldAlert/> Validations en attente ({pendingIngredients.length})</h3>
                    <div className="mt-3 space-y-2">
                        {pendingIngredients.map(ing => (
                            <div key={ing.id} className="bg-white p-3 rounded-lg flex justify-between items-center text-sm shadow-sm">
                                <div><div className="font-bold">{ing.nom}</div><div className="text-xs text-slate-500">{ing.fournisseur}</div></div>
                                <div className="flex gap-2">
                                    <button onClick={()=>validerIngredientAdmin(ing.id, false)} className="p-2 bg-red-50 text-red-600 rounded"><X size={16}/></button>
                                    <button onClick={()=>validerIngredientAdmin(ing.id, true)} className="p-2 bg-green-50 text-green-600 rounded"><CheckSquare size={16}/></button>
                                </div>
                            </div>
                        ))}
                    </div>
        ...
        {showAiModal && (
            <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm max-h-[80vh] overflow-y-auto p-6 animate-in fade-in zoom-in border-t-8 border-purple-600">
                    ...
                    {aiLoading ? (
                        <div className="text-center py-10 space-y-4"><div className="animate-spin text-4xl">🔮</div><p className="text-slate-500 font-bold animate-pulse">Réflexion en cours...</p></div>
                    ) : (
            ...
            <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-in fade-in zoom-in">
                    ...
                    <div className="space-y-3">
                        ...
                        <input className="w-full border p-3 rounded-xl" placeholder="Adresse" value={newFournData.adresse} onChange={e=>setNewFournData({...newFournData, adresse: e.target.value})}/>
                         <div className="flex gap-2">
                        ...
                        <input className="w-full border p-3 rounded-xl" placeholder="Email" value={newFournData.email} onChange={e=>setNewFournData({...newFournData, email: e.target.value})}/>
                        <div className="flex gap-2"><button onClick={()=>setShowCreateFournisseur(false)} className="flex-1 py-3 text-slate-400 font-bold">Annuler</button><button onClick={demanderCreationFournisseur} className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-bold">Envoyer</button></div>
                    </div>
                </div>
            </div>
        )}
        ...
        {showCreateIngredient && (
            <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-in fade-in zoom-in">
                    ...
                    <div className="space-y-3">
                        <input className="w-full border p-3 rounded-xl font-bold" placeholder="Nom *" value={newIngData.nom} onChange={e=>setNewIngData({...newIngData, nom: e.target.value})}/>
                        <div className="flex gap-2">
                             <select className="w-1/2 border p-3 rounded-xl" value={newIngData.famille} onChange={e=>setNewIngData({...newIngData, famille: e.target.value})}>{FAMILLES_LIST.map(f=><option key={f} value={f}>{f}</option>)}</select>
                        ...
                        <select className="w-full border p-3 rounded-xl" value={newIngData.fournisseur} onChange={e=>setNewIngData({...newIngData, fournisseur: e.target.value})}>
                            ...
                            <option value="Solucious">Solucious</option><option value="Bidfood">Bidfood</option><option value="Pochet">Pochet</option>
                            <option value="Autre">Autre...</option>
                        </select>
                        <input className="w-full border p-3 rounded-xl" type="number" placeholder="Prix € HT" value={newIngData.prix} onChange={e=>setNewIngData({...newIngData, prix: e.target.value})}/>
                        <div className="flex gap-2"><button onClick={()=>setShowCreateIngredient(false)} className="flex-1 py-3 text-slate-400 font-bold">Annuler</button><button onClick={demanderCreationIngredient} className="flex-1 bg-green-600 text-white py-3 rounded-xl font-bold">Envoyer</button></div>
                    </div>
                </div>
            </div>
        )}
        ...
        {isCreatingRecette && (
            <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-in fade-in zoom-in">
                    ...
                    <div className="space-y-4">
                        <div><label className="text-[10px] font-bold text-slate-400 uppercase">Nom du plat</label><input type="text" className="w-full border-2 border-slate-200 p-3 rounded-xl font-bold" placeholder="ex: Sandwich Thon Piquant" value={newRecetteName} onChange={e => setNewRecetteName(e.target.value)} /></div>
                        <div><label className="text-[10px] font-bold text-slate-400 uppercase">Type (Pour Marge)</label><select className="w-full border-2 border-slate-200 p-3 rounded-xl font-bold" value={newRecetteCat} onChange={e => setNewRecetteCat(e.target.value)}>{Object.keys(MARGES_FINANCIERES).map(cat => <option key={cat} value={cat}>{cat} (+{MARGES_FINANCIERES[cat].toFixed(2)}€)</option>)}</select></div>
                        <div className="flex gap-2 mt-6"><button onClick={()=>setIsCreatingRecette(false)} className="flex-1 py-3 text-slate-500 font-bold">Annuler</button><button onClick={creerRecette} className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-bold shadow-lg">Créer</button></div>
                    </div>
                </div>
            </div>
        )}
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
                {viewCmd === 'menu' && !commandeDetail && (
                    <div className="space-y-6">
                        ...
                        {/* BOUTON IA (NOUVEAU) */}
                        <button onClick={askGeminiIdeas} className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 text-white p-4 rounded-2xl shadow-lg shadow-purple-200 flex items-center justify-between group hover:scale-[1.02] transition-transform">
                            <div className="flex items-center gap-3">
                                <div className="bg-white/20 p-2 rounded-lg"><Sparkles className="text-white"/></div>
                                <div className="text-left">
                                    <div className="font-bold text-lg">SOS Idées Recettes</div>
                                    <div className="text-xs text-purple-100 opacity-80">Générer un menu avec le stock</div>
                                </div>
                            </div>
